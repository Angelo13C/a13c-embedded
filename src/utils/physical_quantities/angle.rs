...
impl<T: AngleFunctions> AngleGeneric<T>
{
	...
	/// # Examples
	/// ```
	/// # use a13c_embedded::utils::physical_quantities::angle::deg;
	/// #
...
macro_rules! inherit_fn {
	...
	($name: ident, $return_ty: ty) => {
		...
		fn $name(self) -> $return_ty
		{
			...
			#[cfg(not(std))]
			return micromath::F32Ext::$name(self as f32) as Self;
		}
	};
}
macro_rules! impl_angle_functions {
	($type: ty) => {
		impl AngleFunctions for $type
		{
			...
			const FLAT_ANGLE: Self = core::f64::consts::PI as Self;
			...
			fn to_radians(self) -> Self
			{
				self * (Self::FLAT_ANGLE / 180.0 as Self)
			}
			#[inline]
			fn to_degrees(self) -> Self
			{
				const PIS_IN_180: f64 = 57.295_779_513_082_32_f64;
				self * PIS_IN_180 as Self
			}
			...
			{
				#[cfg(std)]
				return self.rem_euclid(Self::ROUND_ANGLE);
				#[cfg(not(std))]
				{
					let r = self % Self::ROUND_ANGLE;
					if r < 0.0
					{
						r + Self::ROUND_ANGLE.abs()
					}
					else
					{
						r
			...
			fn cycles_count(self) -> CyclesCount
			{
				let count = (self / Self::ROUND_ANGLE).abs();
				#[cfg(std)]
				return count.floor() as CyclesCount;
				#[cfg(not(std))]
				return micromath::F32Ext::floor(count as f32) as CyclesCount;
			}
...
impl EpsilonAngle for f32
{
	...
	const EPSILON_ANGLE: Self = 0.000017453292;
}
...
mod tests
{
	...
	fn display_and_debug()
	{
		...
		assert_eq!(format!("{} rad", core::f32::consts::PI), format!("{:#?}", angle));
	}
}
